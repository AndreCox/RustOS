...
use rustos_user::{
    SYS_FS_CLOSE, SYS_FS_OPEN, SYS_FS_READ_HANDLE, SYS_FS_SEEK_HANDLE, syscall1, syscall3,
};
...
fn cstr_matches_lit(s: *const c_char, lit: &[u8]) -> bool {
    if s.is_null() || lit.is_empty() || *lit.last().unwrap_or(&1) != 0 {
        return false;
    }
    let mut i = 0usize;
    loop {
...
pub unsafe extern "C" fn fread(
    ...
    if ptr.is_null() || fp.is_null() || size == 0 {
        return 0;
    }
    let file = unsafe { &mut *(fp as *mut KernelFile) };
    let n = syscall3(
...
#[unsafe(no_mangle)]
pub unsafe extern "C" fn fwrite(_: *const c_void, _: usize, _: usize, _: *mut c_void) -> usize {
    0
}

#[unsafe(no_mangle)]
pub unsafe extern "C" fn fseek(fp: *mut c_void, off: i64, wh: i32) -> i32 {
    let file = unsafe { &mut *(fp as *mut KernelFile) };
...
#[unsafe(no_mangle)]
pub unsafe extern "C" fn fgetc(s: *mut c_void) -> i32 {
    let mut c = 0u8;
    if unsafe { fread(&mut c as *mut u8 as *mut c_void, 1, 1, s) == 1 } {
        c as i32
    } else {
        -1
    }
...
pub unsafe extern "C" fn vfprintf_internal(
    ...
    // Quake assets in some pak sets issue this legacy cvar write; ignore just this noise.
    if cstr_matches_lit(fmt, b"Cvar_Set: variable %s not found\n\0")
        || cstr_matches_lit(fmt, b"Cvar_Set: variable %s not found\0")
    {
        let mut ap_peek = VaListC {
            gp_offset: ap.gp_offset,
            fp_offset: ap.fp_offset,
            overflow_arg_area: ap.overflow_arg_area,
            reg_save_area: ap.reg_save_area,
        };
        let var_name = unsafe { crate::next_gp::<*const c_char>(&mut ap_peek) };
        if cstr_matches_lit(var_name, b"campaign\0") {
            if !dst.is_null() {
                unsafe { *dst = 0 };
            }
            return 0;
    ...
    loop {
        ...
        match unsafe { *f as u8 } {
            ...
            b'd' | b'i' => {
                let mut tmp = [0i8; 32];
                let n = if long_mod {
                    unsafe { crate::next_gp::<i64>(ap) }
                } else {
                    (unsafe { crate::next_gp::<i32>(ap) }) as i64
                };
                ...
                for i in 0..len {
                    write_char!(tmp[i as usize] as u8);
                }
            }
            b'u' => {
                let mut tmp = [0i8; 32];
                let n = if long_mod {
                    unsafe { crate::next_gp::<u64>(ap) }
                } else {
                    (unsafe { crate::next_gp::<u32>(ap) }) as u64
                };
